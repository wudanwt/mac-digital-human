...
class CosyVoiceTTS:
    """Production CosyVoice 2 TTS Engine for ultra-natural digital human lecture speech."""
    ...
    def __init__(self, config: CosyVoiceConfig | None = None) -> None:
        ...
        # Fallback default reference audio & text (wudan)
        default_ref_dir = self.bundle_dir / "voices" / "wudan"
        self._default_ref_audio = default_ref_dir / "reference.wav"
        self._default_ref_text = (
            "今天欢迎来到会议的现场，我很开心，也很荣幸的给大家介绍我们最新的产品。"
        )

    def _get_service(self):
        """Lazy load singleton CosyVoiceService to keep weights warm across lectures."""
        ...
        logger.info(f"Loading CosyVoice 2.0 0.5B model from: {self.bundle_dir}")
        sys.path.insert(0, str(self.bundle_dir))
        try:
            from tts_core.service import CosyVoiceService
            CosyVoiceTTS._shared_service = CosyVoiceService(self.bundle_dir, pause_seconds=self.config.pause_seconds)
            CosyVoiceTTS._shared_bundle_dir = str(self.bundle_dir)
            logger.info("CosyVoice 2.0 model loaded into memory successfully.")
            return CosyVoiceTTS._shared_service
        except Exception as exc:
            logger.error(f"Failed to load CosyVoice model: {exc}")
    ...
    def synthesize(self, text: str, output: Path, **kwargs) -> Path:
        """Synthesize lecture speech using CosyVoice 2 zero-shot clone."""
        ...
        if not ref_text or not ref_text.strip():
            # If ref_text is missing, try loading reference.txt alongside audio
        ...
        try:
            logger.info(f"CosyVoice synthesizing {len(clean_text)} chars with speed={speed}, instruct={instruct}...")
