...
async function renderAdmin(){if(!me.user.is_superuser){$('page').innerHTML='<div class="card">无权限</div>';return}const [o,tenants,orders]=await Promise.all([api('/admin/overview'),api('/admin/tenants'),api('/admin/orders')]);window.__tenants=Object.fromEntries(tenants.map(t=>[t.id,t]));$('page').innerHTML=`<div class="grid stats"><div class="stat"><div class="muted">用户</div><div class="num">${o.users}</div></div><div class="stat"><div class="muted">工作区</div><div class="num">${o.tenants}</div></div><div class="stat"><div class="muted">排队</div><div class="num">${o.queued_jobs}</div></div><div class="stat"><div class="muted">运行</div><div class="num">${o.running_jobs}</div></div><div class="stat"><div class="muted">待确认</div><div class="num">${o.pending_orders}</div></div></div><div class="card" style="margin-top:16px"><h2>工作区</h2><table class="table"><tbody>${tenants.map(t=>`<tr><td>${esc(t.name)}<div class="code">${t.id.slice(0,8)}</div></td><td>${esc(t.plan_code||'-')}</td><td>${t.remaining_minutes} 分钟</td><td><button class="secondary" data-action="credit" data-id="${t.id}">调整额度</button></td></tr>`).join('')}</tbody></table></div><div class="card" style="margin-top:16px"><h2>待确认订单</h2><table class="table"><tbody>${orders.filter(x=>x.status==='pending').map(o=>`<tr><td>${o.id.slice(0,10)}</td><td>${esc(o.plan_code)}</td><td>¥${o.amount_cny}</td><td><button class="primary" data-action="mark-paid" data-id="${o.id}">确认收款</button></td></tr>`).join('')||'<tr><td class="muted">暂无</td></tr>'}</tbody></table></div>`;wirePageActions()}
