...
async function assetMediaUrl(path,onBlob=null){const raw=String(path||''),match=raw.match(/\/assets\/([^/?]+)\/download(?:[?#]|$)/);const url=match?API+'/assets/'+encodeURIComponent(match[1])+'/content':raw.startsWith('/api/')?raw:(API+raw);const r=await fetch(url,{headers:{Authorization:'Bearer '+token}});if(!r.ok)throw Error('预览素材加载失败');const objectUrl=URL.createObjectURL(await r.blob());if(onBlob)onBlob(objectUrl);return objectUrl}
...
async function downloadAsset(id,name){const r=await fetch(API+'/assets/'+encodeURIComponent(id)+'/content',{headers:{Authorization:'Bearer '+token}});if(!r.ok)throw Error('下载失败（HTTP '+r.status+'）');const u=URL.createObjectURL(await r.blob()),a=document.createElement('a');a.href=u;a.download=name||'download';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(u),30000)}
