...
class OrderCreate(BaseModel):
    ...
    provider: str = Field(default="manual", pattern="^(manual|wechat|alipay|mock)$")
...
def apply_paid_order(db: Session, order: PaymentOrder) -> Subscription:
    ...
    sub.period_ends_at = datetime.now(timezone.utc) + timedelta(days=30)
    audit(db, action="billing.plan_activated", tenant_id=order.tenant_id, user_id=order.user_id, target_type="order", target_id=order.id, details={"plan": plan.code})
    return sub
...
def get_subscription(principal: Annotated[Principal, Depends(get_principal)], db: Annotated[Session, Depends(get_db)]) -> dict:
...
def create_order(body: OrderCreate, principal: Annotated[Principal, Depends(get_principal)], db: Annotated[Session, Depends(get_db)]) -> dict:
    ...
    db.add(order)
    db.flush()
    audit(db, action="billing.order_create", tenant_id=principal.tenant_id, user_id=principal.user_id, target_type="order", target_id=order.id, details={"plan": plan.code, "provider": body.provider})
    if body.provider == "mock":
        apply_paid_order(db, order)
    db.commit()
    result = _order_dict(order)
    if body.provider in {"wechat", "alipay"}:
        result["payment_state"] = "provider_credentials_required"
        result["message"] = "Configure merchant credentials and payment callback adapter before accepting real payments."
    elif body.provider == "manual":
        result["payment_state"] = "awaiting_admin_confirmation"
    else:
...
@router.get("/orders")
def list_orders(principal: Annotated[Principal, Depends(get_principal)], db: Annotated[Session, Depends(get_db)]) -> list[dict]:
    orders = db.scalars(select(PaymentOrder).where(PaymentOrder.tenant_id == principal.tenant_id).order_by(PaymentOrder.created_at.desc())).all()
